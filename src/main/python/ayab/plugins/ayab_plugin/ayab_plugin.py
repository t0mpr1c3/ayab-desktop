...
class AyabPlugin(Observable):
    ...
    def __activate_ui(self):
        ...
        self.ui.start_row_edit.valueChanged.connect(
            lambda: self.emit_start_row_updater(
                self.__config.read_start_row(self.ui)))
        ...
        self.ui.alignment_combo_box.currentIndexChanged.connect(
            lambda: self.emit_alignment_updater(
                self.__config.read_alignment(self.ui)))
    ...
    def get_config(self, image):
        ...
        self.__config.read(self.ui)
        self.__logger.debug(self.__config.as_dict())

        # start to knit with the bottom first
        image = image.transpose(Image.ROTATE_180)

        # mirroring option
        if self.__config.auto_mirror:
            image = image.transpose(Image.FLIP_LEFT_RIGHT)

        # TODO: detect if previous conf had the same
        # image to avoid re-generating.

        self.__pattern = AyabPattern(image, self.__config.num_colors)
        if self.__config.start_row > self.__pattern.pat_height:
            self.emit_popup("Start row is larger than the image.")
            self.emit_bad_config_flag()
            return

        # validate configuration options
        valid, msg = self.__config.validate()
        if not valid:
            self.emit_popup(msg)
            self.emit_bad_config_flag()
            return
        ...
        # update pattern
        if self.__config.start_needle and self.__config.stop_needle:
            self.__pattern.set_knit_needles(self.__config.start_needle,
                                            self.__config.stop_needle)
        self.__pattern.alignment = self.__config.alignment

        # update progress bar
        self.emit_progress_bar_updater(self.__config.start_row + 1,
                                       self.__pattern.pat_height, 0, "")

        # switch to status tab
        if self.__config.continuous_reporting:
    ...
    def knit(self):
        ...
        while True:
            # knit next row
            result = self.__control.knit(self.__pattern, self.__config)
