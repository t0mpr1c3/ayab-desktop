...
class KnitControl(object):
    ...
    FIRST_SUPPORTED_API_VERSION = 0x05  # currently this is the only supported version
    ...
    def check_serial_API5(self):
        msg, token, param = self.com.update_API5()
        if token == MessageToken.cnfInfo:
            self.__log_cnfInfo(msg)
        elif token == MessageToken.indState:
            self.status.parse_device_state_API5(param, msg)
        return token, param
    ...
    def cnf_line_API5(self, line_number):
        ...
        color, row_index, blank_line, last_line = self.mode_func(
            self, line_number)
        bits = self.select_needles_API5(color, row_index, blank_line)

        # send line to machine
        self.com.cnf_line_API5(requested_line, bits.tobytes(), last_line
    ...
    def select_needles_API5(self, color, row_index, blank_line):
