...
class AyabCommunicationMockup(AyabCommunication):
    ...
    # def req_test_API5(self) -> None:
    #     cnfTest = bytearray([0xC4, 0x1])
    #     self.__rx_msg_list.append(cnfTest)

    def req_start_API5(self, start_needle, stop_needle, continuous_reporting, machine_val) -> None:
        self.__is_started = True
        cnfStart = bytearray([0xC1, 0x1])
        self.__rx_msg_list.append(cnfStart)

    def cnf_line_API5(self, line_number, line_data, flags) -> bool:
        return True

    def update_API5(self) -> tuple:
