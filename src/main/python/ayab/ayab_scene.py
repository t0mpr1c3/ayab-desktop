...
class Scene (object):
    ...
    def __init__(self, parent):
        ...
        self.__image = None
        self.__alignment = Alignment(parent.prefs.settings.value("default_alignment"))
    ...
    def refresh(self):
        ...
        if self.__alignment.name == 'LEFT':
            pos = self.__start_needle - WIDTH / 2
        elif self.__alignment.name == 'CENTER':
            pos = (self.__start_needle + self.__stop_needle - pixmap.width() - WIDTH) / 2
        elif self.__alignment.name == 'RIGHT':
            pos = self.__stop_needle - pixmap.width() - WIDTH / 2
        else:
        ...
        qscene.addItem(
            QGraphicsRectItem(self.__start_needle - 1 - WIDTH / 2,
                              -self.BAR_HEIGHT,
                              self.LIMIT_BAR_WIDTH,
                              pixmap.height() + 2 * self.BAR_HEIGHT))
        qscene.addItem(
            QGraphicsRectItem(self.__stop_needle - WIDTH / 2,
                              -self.BAR_HEIGHT,
                              self.LIMIT_BAR_WIDTH,
                              pixmap.height() + 2 * self.BAR_HEIGHT))
    ...
    def repeat_image(self):
        ...
        h = QInputDialog.getInt(
            ...
            value=1,
            min=1,
            max=ceil(WIDTH / self.image.size[0])
        )
